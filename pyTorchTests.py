...
# data
x1 = np.linspace(-20, 20, 10000) # Array of numbers in [-20..+20] for f(x)
y1 = np.sin(x1) * x1  # equation №1

x2 = np.linspace(-20, 20, 10000) # Array of numbers in [-20..+20] for g(x)
x2 = torch.tensor(x2).type(torch.float) # wrap array 'x' in tensor
x2 = x2.requires_grad_() # Say system what we want to store gradient

y2 = torch.sin(x2.dot(x2.T)) * x2.dot(x2.T)  # for equation №2
                                            # wtf is '.dot(x.T)' ???

y2.backward()   # take derivative from f(x), I guess
